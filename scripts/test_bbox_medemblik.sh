...
psql -d $DBNAME -v bbox="5.09492, 52.77134, 5.11173, 52.76151, 4326" -f sql/test_bbox.sql
END3=$(python -c'import time; print repr(time.time())')
ECHO "Took: " $(bc -l <<< $END3-$END2)

psql -d $DBNAME -v bbox="5.10725, 52.77271, 5.11791, 52.76455, 4326" -f sql/test_bbox.sql
